...
try:
    ...
    agent = DDPGAgent(critic=Critic, actor=Actor, state_size=state_size, action_size=action_size, 
                    target_network_update_rate=tau, discount_factor=gamma, batch_size=batch_size, buffer_size=exp_replay_buffer_size,
                    num_batches=n_batches_train)
    
    agent.actor.load_state_dict(torch.load('Weights/car/checkpoint_actor.pth'))
    agent.actor_target.load_state_dict(torch.load('Weights/car/checkpoint_actor_target.pth'))
    agent.critic.load_state_dict(torch.load('Weights/car/checkpoint_critic.pth'))
    agent.critic_target.load_state_dict(torch.load('Weights/car/checkpoint_critic_target.pth'))
